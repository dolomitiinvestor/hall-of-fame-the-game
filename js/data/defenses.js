...
// allowed per game (pointsAllowedBonus() in js/scoring.js).
//
// No `tag` field here on purpose: a team defensive unit isn't an
// individual who can be personally enshrined, so HOF/HOVG/ACTIVE don't
// apply -- js/app.js's tagBadge() skips rendering a badge when a
// player has no tag. Position "DEF" is exempt from the draft's
// retired/active alternation regardless (see SKILL_POSITIONS in
// draftEngine.js), so the missing tag has no gameplay effect.
